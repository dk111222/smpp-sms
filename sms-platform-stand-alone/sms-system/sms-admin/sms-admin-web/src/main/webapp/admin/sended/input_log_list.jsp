    ...
    layui.extend({tableExt: '/layuiadmin/extends/tableExt'}).use(['tableExt'], function () {
        ...
        table.render({
            ...
            cols: [[
                ...
                { title: '客户IP/提交时间',templet:function (d) {
                        return d.source_IP + '<br>' + d.input_Date;
                    }, width: 170},
                { title: '审核/分拣时间',templet:function (d) {
                        var a = handleData(d.audit_Status_Code_name)
                        if (d.product_No_ext && d.product_No_ext.name != "---"){
                            a += "/"+handleData(d.product_No_ext.name)
                        }
                        a += '<br>'+d.create_Date;
                        return a;
                    }, width: 170},
                { title: '协议/网关IP',templet:function (d) {
                        var a = handleData(d.protocol_Type_Code_name)+ '<br>';
                        a += handleData(d.gate_Ip);
                        return a;
                    }, minWidth: 150},
            ]]
            ,done: function(res){
