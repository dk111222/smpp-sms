...
public class MASMMSImpl extends AbstractHttp {
	...
	public void submit(Submit submit, IHttpSendCallBack callBack) throws Exception {
        ...
        String modeId = MmsUtil.getMmsChannelTemplateCode(submit.getContent());
        ...
        String signString = buffer.append(modeId).append(timestamp).append(channel.getAccount()).append(channel.getPassword()).toString();
        ...
        httpClient.postAsync(postURL, httpContent, HttpClient.MEDIA_TYPE_JSON, new AbstractCallback(httpClient.getCharset()){
            ...
            public void ok(Call call, CharsetResponseBody response) throws Exception {
                ...
                if(!"0".equals(code)){//0:成功，-1：失败
                    submit.setSubmit_Description("视频短信提交失败：接口返回数据 ,"+resultMap.toString());
                    callBack.failed(submit);
                    return;
                }
