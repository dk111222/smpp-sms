...
public class ReportWrap {
    ...
    private void buildReportAndNotify(String key, Submit submit) {
        //将key缓存，这里防并发检测上游重推
        boolean isDelete = SubmitCacheService.removeSubmitKey(key);
        if (!isDelete) {
            SuperLogger.error("上游重复推送report:" + key);
            return;
        }
        ...
        if (submit == null) {
            ...
            if (submit == null) {
                if (retryTimes >= 0) {
                    ...
                    SuperLogger.error("查不到：" + key + "，等待" + delaySeconds + "后重试");
                }
                return;
            }
        } else {
    ...
    public int getDelaySeconds(int retryTimes) {
        switch (retryTimes) {
            case 4:
                return 3;
            case 3:
