...
public abstract class AbstractSenderService implements ISenderSmsService {
	...
	@Resource(name = "IReportAwaitDAO")
	protected IReportAwaitDAO<ReportAwait> reportAwaitDAO;
	@Autowired
	IInputExtDAO inputExtDAO;
	...
	public boolean saveMO(Inbox inbox) {
		...
		// 手机号使用submit表的，如果没有匹配到submit则使用inbox表的并且86开头的去掉
		if (StringUtils.isNotEmpty(submit.getPhone_No())) {
			inbox.setPhone_No(submit.getPhone_No());
		} else {
			inbox.setPhone_No(StringUtils.removeStart(inbox.getPhone_No().replaceFirst("\\+", ""), "86"));
		}
	...
	@Override
	public List<Submit> getSubmitAwaitAndDel(Long id) {
		SubmitExample example = new SubmitExample();
		SubmitExample.Criteria criteria = example.createCriteria();
		...
		deleteSubmitAwait(submits.get(submits.size() - 1 ).getId());
		return submits;
	}

	@Override
	public List<Input> getInputAwaitAndDel() {
		InputExample example = new InputExample();
		InputExample.Criteria criteria = example.createCriteria();
		example.setPagination(new Pagination(1000));
		List<Input> inputs = this.inputExtDAO.queryInputAwaitList(example);
		if (ObjectUtils.isEmpty(inputs)) {
			return inputs;
		}

		deleteInputAwait(inputs.get(inputs.size() - 1 ). getId());
		return inputs;
	}

	private void deleteInputAwait(Long id) {
		InputAwaitExample example = new InputAwaitExample();
		InputAwaitExample.Criteria criteria = example.createCriteria();
		criteria.andIdLessThanOrEqualTo(id);
		inputAwaitDAO.deleteByExample(example);
	}

	private void deleteSubmitAwait(Long id) {
	...
	@Override
	public List<Report> getReportNotifyAwaitAndDel(Integer userId, Long id, boolean syncDel) {
		...
		synchronized (key.intern()) {
			...
			if (syncDel) {
				deleteReportNotifyAwait(userId, reports.get(reports.size() - 1 ).getId());
				return reports;
			}
	...
	private void deleteReportNotifyAwait(Integer userId, Long id) {
	...
	@Override
	public void saveReportNotifyAwait(List<ReportNotifyAwait> list) {
		reportNotifyAwaitDAO.insertList(list);
	}

	@Override
	public void saveReportAwait(ReportAwait reportAwait) {
		reportAwaitDAO.insertSelective(reportAwait);
	}

	@Override
	public List<ReportAwait> getReportAwaitAndDel(Long id) {
		ReportAwaitExample example = new ReportAwaitExample();
		ReportAwaitExample.Criteria criteria = example.createCriteria();
		criteria.andIdGreaterThan(id);
		example.setOrderByClause(" id");
		example.setPagination(new Pagination(1000));
		List<ReportAwait> reportAwaits = this.reportAwaitDAO.selectByExample(example);
		if (ObjectUtils.isEmpty(reportAwaits)) {
			return reportAwaits;
		}
		deleteReportAwait(reportAwaits.get(reportAwaits.size() - 1).getId());
		return reportAwaits;
	}

	private void deleteReportAwait(Long id) {
		ReportAwaitExample example = new ReportAwaitExample();
		ReportAwaitExample.Criteria criteria = example.createCriteria();
		criteria.andIdLessThanOrEqualTo(id);
		reportAwaitDAO.deleteByExample(example);
	}
}