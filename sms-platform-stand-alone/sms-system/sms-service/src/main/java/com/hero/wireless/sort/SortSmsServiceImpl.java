...
//			SuperLogger.debug(mapKey.getMessageType() + mapKey.getOperator());
	 ...
	 * @version V1.0
	 */
	private void notifySender(SortContext ctx) {
		ctx.getChannelMap().values().stream().forEach(channels -> {
			channels.forEach(sortChannel -> {
				...
				submits.forEach(submit -> {
					...
					Priority priority = Priority.valueOf(ctx.getEnterpriseUser().getPriority_Level());
					submit.setPriority_Level(priority.value());
					//保存到通道发送队列
					notifySender(submit, priority);
				});
			});
		});
	}
	...
	/**
	 * web提交的放入到await表中,协议提交的放入到本地队列
	 * @param submit
	 * @param priority
	 */
	private void notifySender(Submit submit, Priority priority) {
		if (ProtocolType.WEB.toString().equals(submit.getProtocol_Type_Code())) {
			//保存待发送表
