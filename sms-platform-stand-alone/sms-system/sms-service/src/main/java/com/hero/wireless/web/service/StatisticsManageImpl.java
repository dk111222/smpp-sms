...
public class StatisticsManageImpl extends BaseService implements IStatisticsManage {
	...
	public void insertSmsRealTimeStatisticsData(Date startDate, Date endDate) {
		...
		//处理数据
		List<SmsRealTimeStatisticsExt> insertList = dealList(submitData, reportData, startDate);
		//插入
		if (ObjectUtils.isNotEmpty(insertList)) {
			ListUtils.partition(insertList, 2000).forEach(item -> {
				this.smsRealTimeStatisticsDAO.insertList(item);
			});
		}
	}

	//合并list数据
	private List<SmsRealTimeStatisticsExt> dealList(List<SmsRealTimeStatisticsExt> submitData,
													List<SmsRealTimeStatisticsExt> reportData, Date startDate) {
		Map<String, SmsRealTimeStatisticsExt> map = new LinkedHashMap<>();
		for (SmsRealTimeStatisticsExt submit : submitData) {
			submit.setSubmit_Date(startDate);
			submit.setCreate_Date(new Date());
			map.put(makeStatisticsKey(submit), submit);
		}
		for (SmsRealTimeStatisticsExt report : reportData) {
			String key = makeStatisticsKey(report);
			//report和submit可以匹配上
			if (map.get(key) != null) {
				SmsRealTimeStatisticsExt submit = map.get(key);
				submit.setSend_Success_Total(report.getSend_Success_Total());
				submit.setSend_Faild_Total(report.getSend_Faild_Total());
			} else {
				report.setCreate_Date(new Date());
				map.put(key, report);
			}
		}
		List<SmsRealTimeStatisticsExt> insertList = new ArrayList<>();
		map.forEach((k, v) -> {
			insertList.add(v);
		});
		return insertList;
	...
	private String makeStatisticsKey(SmsRealTimeStatisticsExt smsStatistics) {
		if (smsStatistics.getSubmit_Date() == null) {
			return "";
		}

		StringBuffer stringBuffer = new StringBuffer();
		stringBuffer.append(DateTime.getString(smsStatistics.getSubmit_Date(), DateTime.Y_M_D_1))
				.append(smsStatistics.getAgent_No())
				.append((smsStatistics.getBusiness_User_Id() == null ? "" : smsStatistics.getBusiness_User_Id()))
				.append(smsStatistics.getEnterprise_No()).append(smsStatistics.getChannel_No())
				.append(smsStatistics.getEnterprise_User_Id()).append(smsStatistics.getMessage_Type_Code())
				.append(smsStatistics.getCountry_Number())
				.append(smsStatistics.getOperator())
				.append(smsStatistics.getProvince_Code())
				.append(smsStatistics.getSignature() == null ? "" : smsStatistics.getSignature());
