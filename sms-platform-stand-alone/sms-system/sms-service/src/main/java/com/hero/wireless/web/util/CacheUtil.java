...
public class CacheUtil {
    ...
    public static <K, V> Set<V> getMapCachedHashSet(K key, Map<K, Set<V>> map) {
        Set<V> object = map.get(key);
        if (object != null) {
            return object;
        }
        try {
            object = new HashSet<>(16);
        } catch (Exception e) {
            SuperLogger.error("init object failed" + e.getStackTrace());
        }
        Set<V> preObject = map.putIfAbsent(key, object);
        if (preObject != null) {
            object = preObject;
        }
        return object;
