...
public class JinHongMMSImpl extends AbstractHttp {
	...
	public void submit(Submit submit, IHttpSendCallBack callBack) throws Exception {
		...
		String channelTemplateCode = MmsUtil.getMmsChannelTemplateCode(submit.getContent());
		param.put("content", Base64Utils.encode(URLEncoder.encode(channelTemplateCode,"UTF-8")));
		...
		httpClient.postAsync(postURL, param, new AbstractCallback(httpClient.getCharset()) {
			...
			public void ok(Call call, CharsetResponseBody response) throws Exception {
				...
				String result = rootElement.elementText("result");
				if(!result.equals("0")){
					submit.setSubmit_Description("金鸿数字短信提交失败,失败码:"+result);
