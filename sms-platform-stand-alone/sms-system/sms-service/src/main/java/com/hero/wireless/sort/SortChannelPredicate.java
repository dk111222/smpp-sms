...
import com.hero.wireless.sort.diversion.impl.ChannelExcludeKeywordDiversion;
import com.hero.wireless.sort.diversion.impl.ChannelIncludeKeywordDiversion;
...
public class SortChannelPredicate {
    ...
    private boolean contentRule(SortContext ctx, SortChannelMap.SortChannel sortChannel) {
        ...
        boolean includeResult = ((ISortDiversionService.BooleanResult) new IncludeKeywordDiversion().diversion(ctx, sortChannel)).isResult();
        if(!includeResult){
            return includeResult;
        }
        //通道包含关键字检测
        boolean channelIncludeResult = ((ISortDiversionService.BooleanResult) ChannelIncludeKeywordDiversion.getInstance().diversion(ctx, sortChannel)).isResult();
        if (!channelIncludeResult) {
            return channelIncludeResult;
        }
        //通道排除关键字检测
        boolean channelExcludeResult = ((ISortDiversionService.BooleanResult) ChannelExcludeKeywordDiversion.getInstance().diversion(ctx, sortChannel)).isResult();
        if (channelExcludeResult) {
            return false;
        }
        return true;
    }
