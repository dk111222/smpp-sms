...
public class DatabaseCache {
     ...
     * 彩信素材
     */
    private final static LocalCache<MmsMaterial> MMS_MATERIAL_LOCAL_CACHE = new LocalCache<>(10, 1000);
    /**
     * 彩信模板
     */
    private final static LocalCache<MmsTemplate> MMS_TEMPLATE_LOCAL_CACHE = new LocalCache<>(10, 1000);

    /**
     * 通道签名本地缓存，KEY格式: 外面的Map的Key:Channel_NO、里面的map的key:Signature
     */
    public static Map<String, Map<String, List<String>>> CHANNEL_SIGNATURE_MAP = new HashMap<>();

    public static Map<String, Set<String>> CHANNEL_KEY_WORDS_MAP = new HashMap<>();

    public static class IncludeSignature implements ChannelUtil.IDiversionResult {
    ...
    public static Agent getAgentByNo(String agentNo) {
        if (StringUtils.isEmpty(agentNo)) {
            return null;
        }
        AgentExt condition = new AgentExt();
        condition.setNo(agentNo);
        List<Agent> list = databaseCache.agentManage.queryAgentList(condition);
        return ObjectUtils.isEmpty(list) ? null : list.get(0);
    }
    ...
    public static void refreshChannelDiversionLocalCache() {
        SuperLogger.info("======开始通道签名/关键字本地缓存加载======");
        int page = 1;
        int pageSize = 5000;
        while (true) {
            ChannelDiversion channelDiversion = new ChannelDiversion();
            channelDiversion.setStatus_Code(ProductStatus.START.toString());
            List<ChannelDiversion> list = databaseCache.netwayManage.queryChannelDiversionList(page, pageSize, channelDiversion);
            if (ObjectUtils.isEmpty(list)) {
                break;
            }
            list.forEach(entity -> {
                if (ProductChannelDiversionType.SIGNATURE.toString().equals(entity.getStrategy_Type_Code())) {
                    addAndUpdateSignatureCache(entity);
                } else if (ProductChannelDiversionType.INCLUDE_KEYWORD.toString().equals(entity.getStrategy_Type_Code()) ||
                        ProductChannelDiversionType.EXCLUDE_KEYWORD.toString().equals(entity.getStrategy_Type_Code())){
                    addAndUpdateKeyWordsCache(entity);
                }

            });
            page = page + 1;
        }
        SuperLogger.info("======通道签名/关键字本地缓存加载完成=======");
    }

    public static void addAndUpdateKeyWordsCache(ChannelDiversion entity) {
        String key = getKeyWordsCacheKey(entity.getChannel_No(), entity.getStrategy_Type_Code());
        Set<String> setList = CacheUtil.getMapCachedHashSet(key, CHANNEL_KEY_WORDS_MAP);
        setList.add(entity.getStrategy_Rule());
    }

    private static String getKeyWordsCacheKey(String channelNo, String strategyTypeCode){
        return channelNo + strategyTypeCode;
    }

    /**
     * 根据通道编号和导流类型判断内容是否包含关键字
     * @param channelNo 通道编号
     * @param type 导流类型
     */
    public static Set<String> getKeyWordsFromCache(String channelNo, String type) {
        String key = getKeyWordsCacheKey(channelNo, type);
        Set<String> setList = CHANNEL_KEY_WORDS_MAP.get(key);
        return setList;
    }
     ...
     * @param entity
     */
    public static void addAndUpdateSignatureCache(ChannelDiversion entity) {
        IncludeSignature includeSignature = JsonUtil.readValue(entity.getStrategy_Rule(), IncludeSignature.class);
        Map<String, List<String>> map = CHANNEL_SIGNATURE_MAP.get(entity.getChannel_No());
        if (map == null) {
            map = new HashMap<>(16);
            CHANNEL_SIGNATURE_MAP.put(entity.getChannel_No(), map);
        }
        List<String> subCodes = map.get(includeSignature.getSignature());
        if (ObjectUtils.isEmpty(subCodes)) {
            subCodes = new ArrayList<>();
            map.put(includeSignature.getSignature(), subCodes);
        }
        subCodes.add(includeSignature.getSubCode());
    }
    ...
    public static MmsMaterial getMmsMaterialCacheByCode(String code) {
        return MMS_MATERIAL_LOCAL_CACHE.get(code, () -> getMmsMaterialByCode(code));
    }

    public static MmsMaterial getMmsMaterialByCode(String code) {
        if (StringUtils.isEmpty(code)) {
            return null;
        }
        MmsMaterial material = new MmsMaterial();
        material.setMaterial_Code(code);
        List<MmsMaterial> mmsMaterials = databaseCache.materialManage.queryMmsMaterialList(material);
        return ObjectUtils.isEmpty(mmsMaterials) ? null : mmsMaterials.get(0);
    }

    public static MmsTemplate getMmsTemplateCacheByCode(String templateCode) {
        return MMS_TEMPLATE_LOCAL_CACHE.get(templateCode, () -> getMmsTemplateByCode(templateCode));
    }

    public static MmsTemplate getMmsTemplateByCode(String templateCode) {
        if (StringUtils.isEmpty(templateCode)) {
            return null;
        }
        MmsTemplate template = new MmsTemplate();
        template.setTemplate_Code(templateCode);
        List<MmsTemplate> templates = databaseCache.materialManage.queryMmsTemplateList(template);
        return ObjectUtils.isEmpty(templates) ? null : templates.get(0);
    }
     ...
     * @return
     */
    public static List<EnterpriseUser> getEnterpriseUserByAgentNo(String agentNo, String status) {
        if (StringUtils.isBlank(agentNo)) {
            SuperLogger.error("参数agentNo不能为空");
        ...
        if (StringUtils.isNotBlank(status)) {
            enterpriseExt.setStatus(status);
        }
        List<? extends Enterprise> enterpriseList = databaseCache.enterpriseManage.queryEnterpriseList(enterpriseExt);
        ...
        return databaseCache.enterpriseManage.queryEnterpriseUserList(enterpriseUserExt);
    }

    public static List<String> getEnterpriseNoByAgentNo(Agent agent) {
        List<String> enterpriseNoList = new ArrayList<>();
        EnterpriseExt enterpriseExt = new EnterpriseExt();
        enterpriseExt.setAgent_No(agent.getNo());
        List<Enterprise> enterpriseList = databaseCache.enterpriseManage.queryEnterpriseList(enterpriseExt);
        for (Enterprise enterprise : enterpriseList) {
            enterpriseNoList.add(enterprise.getNo());
        }
        if (enterpriseNoList.size() < 1) {
