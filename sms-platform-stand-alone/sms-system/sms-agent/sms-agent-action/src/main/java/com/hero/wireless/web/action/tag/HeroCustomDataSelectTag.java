...
public class HeroCustomDataSelectTag extends TagSupport {
	...
	private String paramType;// 自定义参数标志 agentNo、enterpriseNo ...其他扩展等
	...
	private List<TagOption> dataSource() {
		...
		case "allEnterpriseUsers":
			...
			if ("agentNo".equals(this.paramType)) {
				userList = DatabaseCache.getEnterpriseUserByAgentNo(param, null);
			} else if ("enterpriseNo".equals(this.paramType)) {
				// 查询企业用户
				enterpriseUserExt.setEnterprise_No(param);
				userList = DatabaseCache.getEnterpriseUserList(enterpriseUserExt);
			} else {
				userList = DatabaseCache.getEnterpriseUserList(enterpriseUserExt);
			}
