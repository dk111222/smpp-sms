...
layui.define(['table', 'form'], function (exports) { //提示：模块也可以依赖其它模块，如：layui.define('layer', callback);
    ...
    tableExt.render = function (params) {
        ...
        if (params.extField) {
            ...
            //加载组件所需样式
            layui.link('/js/layui-ext/layuiExt.css?v=1', function () {
    ...
    function extendData(data, field) {
        ...
        for (var i = 0; i < size; i ++) {
            ...
            this.addTR = $([
                ...
                + showContentByType(data[i],field) +
                "</div>"
                , "</td></tr>"].join("")
            );
            tr.after(this.addTR);
    ...
    //根据message_Type_Code判断展示内容，普通短信正常展示，彩信展示彩信预览按钮行
    function showContentByType(dataRow,field) {
        //后期有5G短信的预览则加上 dataRow.message_Type_Code=="rcs" --encodeURIComponent(dataRow[field])
        if(dataRow.message_Type_Code=="mms" ||dataRow.message_Type_Code=="video"){

            return "<a href='javascript:;' lay-event='{\"type\":\"dialogNoBtn\",\"width\":370,\"height\":590,\"url\":\"/common/mms_preview.jsp?&mmsContent="+encodeURIComponent(dataRow[field])+"\",\"title\":\"彩信内容\"}' style='color: #01aaed;text-decoration-line: underline;'>"+"彩信预览"+"</a>";
