...
<script>
    ...
    function merge(res,$) {
        ...
        for (var k = 0; k < columsName.length; k++) { //这里循环所有要合并的列
            ...
            for (var i = 1; (!!res.data) && i < res.data.length; i++) { //这里循环表格当前的数据
    ...
    function multDownloadFile() {
        layer.confirm('确认批量下载所有查询结果中的文件？', {
            btn: ['确认', '取消'] //按钮
        }, function (index) {
            window.open('/admin/sended_multDownloadFile?' + $("#layuiForm").serialize());
            layer.close(index);
        });
    }
</script>