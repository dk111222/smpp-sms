...
import com.hero.wireless.http.connector.AliVoiceImpl;
...
public class AliController {
    ...
    @ResponseBody
    @RequestMapping("/voice/report")
    public String voiceReport(@RequestBody String jsonArray) {
        try {
            SuperLogger.debug(jsonArray);
            List<AliVoiceReport> aliVoiceReportList = JsonUtil.STANDARD.readValue(jsonArray,new TypeReference<List<AliVoiceReport>>() {});
            new AliVoiceImpl().report(new ChannelReport(null, aliVoiceReportList));
            return (new AliVoiceResponse(0, "接收成功").toJson());
        } catch (Exception e) {
            SuperLogger.error(e.getMessage(), e);
            return (new AliVoiceResponse(1, "接收失败").toJson());
        }
    }
